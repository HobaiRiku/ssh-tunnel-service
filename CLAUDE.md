...
The home's permission model is a `paths.Perm` chosen per-platform by `homePerm` (alongside `platformDefaultHome` in each `home_*.go`) and applied by `EnsureTree`. The default everywhere is **owner-only** (`0700` dirs, `0600` files, no chown). macOS overrides it for the *elevated* system service: the tree is chowned to group `admin` with `0755` dirs and a `0664` `config.yaml` so admin users can edit config without `sudo` — but the API `token` and private `keys/` keep `paths.FileMode()` (`Secret`, `0600`) and never relax. Config writes use `paths.ConfigMode()` (not `FileMode()`); `config.WriteRaw` chmods to the exact mode so the umask can't strip the group-write bit. New configs inherit the home's group (BSD semantics); `EnsureTree` also re-chowns an existing `config.yaml` so upgrades from the old private layout pick up the relaxed perms.
